function PostModeMenu() {
	...
	if(mode=="view"){
		...
		$(".thumb a").each(function(){
			...
			$(this).attr("href", "http://" + document.domain + "/board/post/view/" + id);
		});
	}
}
...
function set_tags(id, tags){
	...
	if((tags!="") && (tags!=null)){
		$.ajax({
			type: "POST",
			cache: false,
			url: "http://" + document.domain + "/board/ajax/image/edit",
...
function get_info(id){
	...
	$.ajax({
		type: "GET",
    	url: "http://www.furpiled.com/api/get_posts",
		data: "username=seinkraft&password=c1e02410475cd7d8cccf62bd9f7fca66&tags=" + id,
		timeout: 40000,
		dataType: "xml",
		encoding:"UTF-8",
...
function set_status(id, status){
	...
	if(((status=="l") || (status=="a") || (status=="p") || (status=="d")) && (id!=null)){
		$.ajax({
		   type: "POST",
		   cache: false,
		   url: "http://" + document.domain + "/board/ajax/image/status",
...
function favorite(id, favorite){
	...
	if(((favorite=="set") || (favorite=="unset")) && (id!=null)){
		$.ajax({
		   	type: "POST",
			cache: false,
		   	url: "http://" + document.domain + "/board/ajax/image/favorite",
...
function vote(id, vote){
	...
	if(((vote=="up") || (vote=="down")) && (id!=null)){
		$.ajax({
		   	type: "POST",
			cache: false,
		   	url: "http://" + document.domain + "/board/ajax/image/vote",
...
function rate(id, rate){
	...
	if(((rate=="s") || (rate=="q") || (rate=="e")) && (id!=null)){
		$.ajax({
			type: "POST",
			cache: false,
		   	url: "http://" + document.domain + "/board/ajax/image/rate",
...
function report(id, reason){
	$.ajax({
	   type: "POST",
	   cache: false,
	   url: "http://" + document.domain + "/board/ajax/image/report",
